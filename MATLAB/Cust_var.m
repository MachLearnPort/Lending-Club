...
X_MAP_N = X_MAP;
X_opt_N = X_opt;
mu = zeros(1, size(X_MAP, 2));
sigma = zeros(1, size(X_MAP, 2));
mu=mean(X_MAP);
sigma=std(X_MAP);
for i=1:size(X_MAP,2)
    X_MAP_N(:,i)=1+(X_MAP(:,i)-mu(i))/sigma(i); %Normailzed about 1
    X_opt_N(:,i)=1+(X_opt(:,i)-mu(i))/sigma(i); %Normailzed about 1
end
...
%Determine Variances
for i=1:size(X_MAP,2)
    X_var(:,i)=abs(X_MAP_N(:,i) - X_opt_N(i)); %Variance from the optimal configuration
end
...
%Remove any NaN and inf fragments from X_var
X_var(isnan(X_var)) = 0;
X_var(isinf(X_var)) = 0;
